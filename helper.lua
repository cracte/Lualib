...
LogFile={
AssetsLog = "AssetsLog",--;     "资源bundle相关的log日志"
LaunchLog = "LaunchLog",--;  “启动过程相关日志”
PatchsLog = "PatchsLog",--;       “更新升级相关日志”
RunningLog = "RunningLog",--;  “运行时日志”
Error = "ErrorLog",--;                        “错误相关日志”
UnHandleException = "UnHandleException",--;   “未捕获的异常相关日志”
Exception = "Exception",--;            “异常日志”
MemoryLog = "MemoryLog",--;      “内存相关日志”
DefaultLogFileName = "default",--;   “默认日志“
}


local CommonLog_ins = luanet.import_type("Common.ULogFile").sharedInstance
local  CommonUDebug = luanet.import_type("Common.UDebug")

print_e = function (str,type_)
    str = str or "null"
    if(type_)then
        CommonLog_ins:LogErrorEx(type_,str)
    else
        CommonLog_ins:LogErrorEx("default",str)
       -- CommonUDebug.LogError(str)
    end
end

print_w = function (str,type_)
    str = str or "null"
    if(type_)then
        CommonLog_ins:LogWarningEx(type_,str)
    else
        CommonLog_ins:LogWarningEx("default",str)
       -- CommonUDebug.LogWarning(str)
    end
end

print = function (str,type_)
    str = str or "null"
    if(type_)then
        CommonLog_ins:LogEx(type_,str)
    else
        CommonLog_ins:LogEx("default",str)
       -- CommonUDebug.Log(str)
    end
...
local ObjTool = luanet.import_type("UniScene.ObjTool");

msg2str = function ( __msg )
    return ObjTool.ToString(__msg)
end

local LuaPatch = luanet.import_type("NLua.LuaPatch");
function Int64ToString( _number64 )
   return LuaPatch.GetInt64String(_number64)
end
